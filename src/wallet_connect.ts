...
export class ErmisAuth {
  ...
  userAgent?: string;
  constructor(key: string, address: string, options?: ErmisChatOptions) {
    this.key = key;
  ...
  public static getInstance(key: string, address: string, options?: ErmisChatOptions) {
    if (!ErmisAuth._instance) {
      ErmisAuth._instance = new ErmisAuth(key, address, options);
    }
    return ErmisAuth._instance as ErmisAuth;
  }
  ...
  async startAuth(): Promise<any> {
    const response = await this.post<StartAuthResponse>(this.baseURL + '/wallets/auth/start', {
